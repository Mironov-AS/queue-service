...
function runAutoReset() {
	...
	const d = today();
	const lastReset = db
		.prepare("SELECT value FROM settings WHERE key='auto_reset_last_date'")
		.get();
	if (lastReset?.value === d) return;
	db.prepare(
		"UPDATE tickets SET status='served', served_at=CURRENT_TIMESTAMP WHERE date=? AND status IN ('waiting','called')",
	).run(d);
	const lastTicket = db
		.prepare("SELECT MAX(id) AS max_id FROM tickets WHERE date=?")
		.get(d);
	db.prepare(
		"INSERT OR REPLACE INTO settings (key, value) VALUES ('queue_reset_last_id', ?)",
	).run(String(lastTicket?.max_id || 0));
	db.prepare(
		"INSERT OR REPLACE INTO settings (key, value) VALUES ('auto_reset_last_date', ?)",
	).run(d);
...
function runAutoOpen() {
	...
	const timeSetting = db
		...
		.get();
	const openTime = timeSetting?.value || "09:00";
	const now = new Date();
	const hh = String(now.getHours()).padStart(2, "0");
	const mm = String(now.getMinutes()).padStart(2, "0");
	const currentTime = `${hh}:${mm}`;
	if (currentTime !== openTime) return;
	const lastOpen = db
		.prepare("SELECT value FROM settings WHERE key='auto_open_last_date'")
		.get();
	const todayStr = today();
	if (lastOpen?.value === todayStr) return;
	const regRow = db
		.prepare("SELECT value FROM settings WHERE key='registration_open'")
		.get();
	if (regRow?.value !== "1") {
		db.prepare(
			"UPDATE settings SET value = '1' WHERE key = 'registration_open'",
		).run();
		console.log(`[auto-open] Registration auto-opened at ${currentTime}`);
		const { getIo } = require("./services/socketSetup");
		const io = getIo();
		if (io) io.emit("registration:changed", { open: true });
	}
	db.prepare(
		"INSERT OR REPLACE INTO settings (key, value) VALUES ('auto_open_last_date', ?)",
	).run(todayStr);
}

setInterval(runAutoReset, 60 * 1000);
setInterval(runAutoOpen, 60 * 1000);
