...
function ManualRegModal({ services, onClose, onCreated }) {
	const defaultService =
		services.find((s) => s.is_default) ||
		(services.length === 1 ? services[0] : null);
	const [serviceId, setServiceId] = useState(
		defaultService ? String(defaultService.id) : "",
	);
	...
	const submit = async () => {
		...
		const hasCheckFields = serviceFields.some(
		...
		if (hasCheckFields && fvArray.length > 0) {
			const checkRes = await fetch("/api/service-fields/check-duplicate", {
				...
				body: JSON.stringify({ service_id: serviceId, field_values: fvArray }),
			});
			if (checkRes.ok) {
				...
				{duplicateTicket && (
					<div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
						...
						<div className="bg-white rounded-lg p-3">
							<p className="text-xs text-gray-500">Найден активный талон</p>
						...
						<button
							...
							className="w-full text-gray-500 hover:text-gray-700 text-sm py-1 transition"
						>
							Ввести другой номер
						</button>
					</div>
...
function EditTicketModal({
	...
	useEffect(() => {
		const storedFv = Array.isArray(ticket.field_values)
			? ticket.field_values
			: [];
		...
		fetch(`/api/services/${form.service_id}/fields`)
			...
			.then((fields) => {
				...
				const orphaned = storedFv.filter(
					(fv) => fv.value && !templateIds.has(Number(fv.field_id)),
				);
				orphaned.forEach((fv) => {
					init[`o_${fv.field_id}`] = fv.value || "";
				});
				...
				{windowsCount > 1 && (
					<div>
						...
						<div className="grid grid-cols-5 gap-1.5">
							{Array.from({ length: windowsCount }, (_, i) => i + 1).map(
								(w) => (
									<button
										key={w}
										type="button"
										onClick={() =>
											setForm((f) => ({
												...f,
												window_number: f.window_number === w ? "" : w,
											}))
										}
	...
	return (
		<Modal title={title || "Выберите окно"} onClose={onClose}>
			<div className="space-y-3">
				...
				<button
					onClick={onClose}
					className="w-full border border-gray-200 rounded-xl py-2.5 text-gray-600 hover:bg-gray-50 text-sm"
				>
					Отмена
				</button>
			</div>
...
export default function QueueTab() {
	...
	const currentList = Array.isArray(queue.current)
		? queue.current
		: queue.current
			? [queue.current]
			: [];
	...
	useEffect(() => {
		...
		return () => {
			socket.off("queue:updated", setQueue);
	...
	const doCallNext = (window) => {
		...
		queueAction("/api/queue/next", "POST", body, {
	...
	return (
		<div className="space-y-5">
				...
				{currentList.length > 0 ? (
					<div className="space-y-4">
						{currentList.map((ticket) => (
							<div
								key={ticket.id}
								className={`flex flex-wrap items-start gap-4 ${currentList.length > 1 ? "border-b border-gray-100 pb-4 last:border-0 last:pb-0" : ""}`}
							>
								<div className="flex-1 min-w-0">
									...
									{ticket.phone && (
										<div className="text-sm text-gray-400">{ticket.phone}</div>
									)}
								...
								<div className="flex flex-wrap gap-2">
									...
									<button
										onClick={() => returnToQueue(ticket.id)}
										disabled={loading}
					...
					<div className="space-y-2 max-h-[32rem] overflow-y-auto">
						{queue.waiting.map((t, i) => {
							...
							return (
								<div
									key={t.id}
									className={`px-4 py-3 rounded-xl ${i === 0 ? "bg-blue-50 border border-blue-200" : "bg-gray-50"}`}
								>
									<div className="flex items-start gap-3">
										<span
											className={`text-2xl font-black w-14 shrink-0 pt-0.5 ${i === 0 ? "text-blue-700" : "text-gray-700"}`}
										>
											№{t.number}
										</span>
										<div className="flex-1 min-w-0">
										...
										<div className="flex items-center gap-2 shrink-0 pt-0.5">
											{t.avg_duration_minutes > 0 && (
												<span className="text-xs text-gray-400 flex items-center gap-1">
													<Icon d={P.clock} cls="w-3.5 h-3.5" />~
													{(i + 1) * t.avg_duration_minutes} мин
												</span>
											)}
											<button
												onClick={() => setEditTicket(t)}
												title="Редактировать талон"
											...
											<button
												...
												disabled={loading}
												className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-green-700 bg-green-50 hover:bg-green-100 rounded-lg disabled:opacity-40 transition"
												title="Вызвать этого клиента"
											>
			...
			{callConfirm && (
				<Modal
					title={`Вызвать талон №${callConfirm.number}`}
					onClose={() => setCallConfirm(null)}
				>
					<div className="space-y-4">
						<p className="text-sm text-gray-600">
							Сейчас обслуживается <strong>№{currentList[0]?.number}</strong>.
							Текущий посетитель будет отмечен как обслуженный, а{" "}
							<strong>№{callConfirm.number}</strong> вызван внеочередно.
						</p>
						<div className="flex gap-2">
							<button
								onClick={doCallSpecific}
			...
			{windowPicker && (
				<WindowPickerModal
					...
					title={
						windowPicker.ticket
							? `Вызвать №${windowPicker.ticket.number} — выберите окно`
							: "Вызвать следующего — выберите окно"
					}
				/>
			)}
		</div>
