...
export default function QRTab() {
	...
	const [terminalQr, setTerminalQr] = useState(null);
	...
	useEffect(() => {
		...
		fetch(`/api/qrcode?url=${encodeURIComponent(dashUrl)}`)
			.then((r) => r.json())
			.then(setDashboardQr);
		const termUrl = window.location.origin + "/terminal";
		fetch(`/api/qrcode?url=${encodeURIComponent(termUrl)}`)
			.then((r) => r.json())
			.then(setTerminalQr);
	}, []);
		...
		<div className="space-y-6 max-w-lg">
			...
			<div className="bg-gray-950 rounded-2xl p-6 flex flex-col gap-5">
				<div>
					...
					<p className="text-gray-400 text-sm mt-1">
						Установите на терминале в зале ожидания — клиенты получают талон
						самостоятельно с обратным отсчётом.
					</p>
				</div>
				<div className="flex gap-3">
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition"
					>
						<Icon d={P.eye} cls="w-4 h-4" /> Открыть терминал
					</a>
				</div>
				{terminalQr && (
					<div className="flex gap-6 items-center flex-wrap">
