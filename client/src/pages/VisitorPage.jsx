...
function GetTicket({ preService, clientId }) {
	...
	useEffect(() => {
		...
		const servicesUrl = clientId
			? `/api/services?client_id=${encodeURIComponent(clientId)}`
			: "/api/services";
		fetch(servicesUrl)
			.then((r) => r.json())
			.then((data) => {
	...
	useEffect(() => {
		...
		fetch(`/api/services/${selected}/fields`)
			.then((r) => r.json())
			.then((data) => {
				setFields(data);
							...
							<div>
								{services.length === 1 ? (
									<div className="flex items-center justify-between p-4 rounded-2xl border-2 border-blue-600 bg-blue-50">
										...
										{services[0].avg_duration_minutes > 0 && (
											<span className="text-xs text-gray-400 shrink-0 ml-2">
												~{services[0].avg_duration_minutes} мин
											</span>
										)}
									</div>
								) : (
										...
										<div className="space-y-2">
											{services.map((s) => (
												...
												>
													<div>
														<span
															className={`font-semibold text-sm ${selected === String(s.id) ? "text-blue-700" : "text-gray-800"}`}
														>
															{s.name}
														</span>
														{s.description && (
							...
							{duplicateTicket && (
								<div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-3">
									...
									<button
										onClick={() => {
											const url = clientId
												? `/visitor?ticket=${duplicateTicket.id}&client_id=${encodeURIComponent(clientId)}`
												: `/visitor?ticket=${duplicateTicket.id}`;
											navigate(url, { replace: true });
										}}
										className="w-full bg-amber-500 hover:bg-amber-600 text-white font-semibold py-2.5 rounded-xl text-sm transition"
									>
										Посмотреть статус талона
									</button>
									<button
...
function TicketStatus({ ticketId, clientId }) {
	...
	const cancelTicket = async () => {
		if (!confirm("Отменить талон? Ваше место в очереди будет потеряно."))
			return;
		...
		return (
			<div className="min-h-screen bg-gradient-to-br from-gray-500 to-gray-700 flex flex-col items-center justify-center p-6">
				<div className="w-full max-w-sm text-center space-y-5">
					...
					<button
						onClick={() => navigate("/visitor", { replace: true })}
	...
	if (ticket.status === "served" && showThankYou)
		return (
	...
	if (
		...
		(ticket.status === "served" && !showThankYou)
	)
		return (
			<div className="min-h-screen bg-gradient-to-br from-green-500 to-emerald-700 flex flex-col items-center justify-center p-6">
				<div className="w-full max-w-sm text-center space-y-5">
						...
						{ticket.window_number ? (
							<p className="text-green-100 mt-1">
								...
								<span className="font-bold text-white">
									окну №{ticket.window_number}
								</span>
							</p>
						) : (
							<p className="text-green-100 mt-1">
								Пожалуйста, подойдите к специалисту
							</p>
						)}
					</div>
					<div className="bg-white rounded-3xl p-8 shadow-2xl">
						...
						<div className="text-8xl font-black text-green-600 leading-none">
							№{ticket.number}
						</div>
	...
	const progressPct =
		totalWaiting > 1
			? Math.max(0, 100 - ((pos - 1) / totalWaiting) * 100)
			: pos <= 1
				? 80
				: 0;
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-blue-600 to-blue-900 flex flex-col items-center justify-center p-5">
			<div className="w-full max-w-sm space-y-4">
							...
							<div className="mt-4 pt-4 border-t border-gray-100 text-left space-y-1">
								{ticket.field_values.map((fv, i) => (
									<div key={i} className="flex justify-between text-sm">
										...
										<span className="text-gray-700 font-medium ml-4 text-right">
											{fv.value || "—"}
										</span>
									</div>
								))}
				...
				{/* Stats */}
				<div
					className={`grid gap-3 ${showWait ? "grid-cols-2" : "grid-cols-1"}`}
				>
