...
router.post('/:id/fields', requireAuth, (req, res) => {
  ...
  ).run(id, label.trim(), ft, required ? 1 : 0, require_check ? 1 : 0, maxOrder.m + 1);
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  db.prepare(
    ...
    require_check !== undefined ? (require_check ? 1 : 0) : (field.require_check || 0),
...
router.post('/check-duplicate', (req, res) => {
  ...
  if (!service_id || !Array.isArray(field_values) || field_values.length === 0) {
    return res.json({ duplicate: false, ticket: null });
  }
  
  // Получаем поля услуги с require_check = 1
  const checkFields = db.prepare(
  ...
  const valuesToCheck = field_values.filter(fv => checkFieldIds.has(fv.field_id));
  ...
  for (const ticket of activeTickets) {
    ...
    for (const checkField of valuesToCheck) {
      ...
      const ticketValue = (ticketField.value || '').toString().toLowerCase().trim();
      const checkValue = (checkField.value || '').toString().toLowerCase().trim();
