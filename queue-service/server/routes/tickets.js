...
router.post("/", ticketLimiter, (req, res) => {
	...
	if (service_id) {
		const requiredFields = db
			.prepare(
				"SELECT id, label FROM service_fields WHERE service_id = ? AND required = 1",
			)
			.all(service_id);
	...
	if (regRow?.value !== "1") {
		return res
			.status(403)
			.json({ error: "Самостоятельная запись временно недоступна" });
	}

	if (service_id) {
		const svc = db
			.prepare(
				"SELECT * FROM services WHERE id = ? AND active = 1 AND enabled = 1",
			)
			.get(service_id);
		...
		if (svc.daily_limit) {
			const todayCount = db
				.prepare(
					"SELECT COUNT(*) AS c FROM tickets WHERE date = ? AND service_id = ?",
				)
				.get(d, service_id);
			if (todayCount.c >= svc.daily_limit) {
				return res
					.status(400)
					.json({ error: "Достигнут дневной лимит талонов для этой услуги" });
			}
		}
	}
  ...
  `)
	...
	res.json({
		...ticket,
		field_values: ticket.field_values ? JSON.parse(ticket.field_values) : [],
		position,
	});
});
...
router.post("/manual", requireAuth, (req, res) => {
	...
	// Validate required service fields for manual registration too
	if (service_id) {
		const requiredFields = db
			.prepare(
				"SELECT id, label FROM service_fields WHERE service_id = ? AND required = 1",
...
router.get("/:id", (req, res) => {
  ...
  `)
	...
	if (ticket.status === "waiting") {
		...
		estimatedWait =
			ticket.avg_duration_minutes > 0
				? position * ticket.avg_duration_minutes
				: null;
	}
...
router.put("/:id", requireAuth, (req, res) => {
	...
	const { name, phone, service_id, is_priority, field_values, status } =
		req.body;
	...
	const svcId =
		service_id !== undefined ? parseId(service_id) || null : ticket.service_id;
	const newName =
		name !== undefined
			? typeof name === "string"
				? name.trim() || null
	...
	const newPhone =
		phone !== undefined
			...
			: ticket.phone;
	const newIsPriority =
		is_priority !== undefined ? (is_priority ? 1 : 0) : ticket.is_priority;
	const newFieldValues =
		field_values !== undefined
			? Array.isArray(field_values) && field_values.length
  ...
  `)
	...
	res.json({
		...updated,
		field_values: updated.field_values ? JSON.parse(updated.field_values) : [],
	});
});
...
router.put("/:id/transfer", requireAuth, (req, res) => {
  ...
  `)
	...
	log(
		req,
		"ticket.transferred",
		`#${ticket.number}: ${ticket.service_name} → ${newSvc?.name}`,
	);
	emitQueueUpdate();
	res.json(
		db
...
router.get("/", requireAuth, (req, res) => {
	...
	const VALID_STATUSES = ["waiting", "called", "served"];
	const status =
		req.query.status && VALID_STATUSES.includes(req.query.status)
			? req.query.status
			: null;
